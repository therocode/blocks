...
asIScriptObject* EntityInterface::createEntity(const std::string& type, float x, float y, float z)
{
    ...
    {
        mBus.sendMessage<LogMessage>(LogMessage("Script runtime error: Id of created entity of type '" + type + "' is invalid. Abandoning script initialisation", mLogName, LogLevel::ERR));
        return nullptr;
    }
