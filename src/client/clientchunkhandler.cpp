#include "clientchunkhandler.hpp"

ClientChunkHandler::ClientChunkHandler(fea::MessageBus& bus) :
    mBus(bus)
{
    subscribe(mBus, *this);
}
...
void ClientChunkHandler::handleMessage(const ClientChunksDeliveredMessage& received)
{
...
void ClientChunkHandler::handleMessage(const VoxelUpdatedMessage& received)
{
...
void ClientChunkHandler::handleMessage(const ClientChunkDeletedMessage& received)
{
    mLocalChunks.erase(received.coordinate);
}

void ClientChunkHandler::updateChunk(const ChunkCoord& coordinate)
{
