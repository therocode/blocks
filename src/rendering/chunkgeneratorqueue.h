...
class ChunkGeneratorQueue{
    public:
        ...
        void generateSomeChunks(std::unordered_map<ChunkCoordinate, VBO>& chunkMap, const VoxelWorldCoordinate focusPoint);
        void setMaxGenTime(int milliseconds);
    private:
        Timer mTimer;
        int mMaxGenTime = 10;
        VBOCreator mCreator;
        std::unordered_map<ChunkCoordinate, ChunkQueueThing> mQueue;
};
